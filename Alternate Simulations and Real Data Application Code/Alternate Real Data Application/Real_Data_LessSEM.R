...
Country.Results <- readRDS(paste0(Country, ".Alt2_Results.rds"))
...
for(i in 1:m_hat){
  for(j in 1:22){
    Ynewnames[j+(i - 1)*22] <- paste0("y", j+(i - 1)*22, "*", colnames(Y.CountrCountry <- gsub("_", " ", Country)
y.train)[j])
  }
}
...
lavmodel <- c(Xmodel, Ymodel, Zmodel, Wcov)
lavdat <- cbind(X.Country.train, Y.Country.train)

lav_mod <- sem(lavmodel, data = lavdat, meanstructure = T, orthogonal = T)

lessSEM_mod <- adaptiveLasso(lav_mod, regularized = c(Xfree, Yfree), lambdas = seq(0, 15, 0.1), control = lessSEM::controlGlmnet(nCores = 25))

saveRDS(lessSEM_mod, file=paste(Country,".lessSEM.Results.rds", sep=""))