...
EZest <- function(A, A0, B, B0, Gamma, Phi1, Phi2, Phi3, Psi, X, Y){
  q <- ncol(X)
  p <- ncol(Y)
  s <- ncol(B)
  m <- ncol(A)
...
for(Country in Countries){
  ...
  Output <- readRDS(paste0(Country, ".Alt2_Results.rds"))
  ...
  EZ.test <- EZest(Output$A_estimates$A_final_pars$A, Output$A_estimates$A_final_pars$A0, Output$B_estimates$B_final_pars$B, Output$B_estimates$B_final_pars$B0,
                   Output$A_estimates$A_final_pars$Gamma, Output$B_estimates$B_final_pars$Phi1, Output$A_estimates$A_final_pars$Phi2, Output$A_estimates$A_final_pars$Phi3, Output$B_estimates$B_final_pars$Psi, X.Country.test, Y.Country.test)
